...
class CoreMoodlenetGetShareInfoActivity200Response(BaseModel):
    """
    CoreMoodlenetGetShareInfoActivity200Response
    """ # noqa: E501
    issuerid: StrictInt = Field(description="MoodleNet issuer id")
    name: StrictStr = Field(description="Activity name")
    server: StrictStr = Field(description="MoodleNet server")
    status: StrictBool = Field(description="status: true if success")
    supportpageurl: StrictStr = Field(description="Support page URL")
    type: StrictStr = Field(description="Activity type")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "issuerid": obj.get("issuerid") if obj.get("issuerid") is not None else null,
            "name": obj.get("name") if obj.get("name") is not None else 'null',
            "server": obj.get("server") if obj.get("server") is not None else 'null',
            "status": obj.get("status"),
            "supportpageurl": obj.get("supportpageurl") if obj.get("supportpageurl") is not None else 'null',
