/*
 * Moodle Webservice API
 *
 * Auto-generated OpenAPI spec for Moodle's Webservice API. Requires installation of the Catalyst RESTful API plugin: https://github.com/catalyst/moodle-webservice_restful.
 *
 * The version of the OpenAPI document: 4.3.4
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct ModGlossaryGetCategories200ResponseCategoriesInner {
    /// The glossary ID
    #[serde(rename = "glossaryid", skip_serializing_if = "Option::is_none")]
    pub glossaryid: Option<i32>,
    /// The category ID
    #[serde(rename = "id", skip_serializing_if = "Option::is_none")]
    pub id: Option<i32>,
    /// The name of the category
    #[serde(rename = "name", skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Whether the category is automatically linked
    #[serde(rename = "usedynalink", skip_serializing_if = "Option::is_none")]
    pub usedynalink: Option<bool>,
}

impl ModGlossaryGetCategories200ResponseCategoriesInner {
    pub fn new() -> ModGlossaryGetCategories200ResponseCategoriesInner {
        ModGlossaryGetCategories200ResponseCategoriesInner {
            glossaryid: None,
            id: None,
            name: None,
            usedynalink: None,
        }
    }
}

